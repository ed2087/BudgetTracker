...
async function processReceiptFile(file) {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (result) {
        result.innerHTML = '<p class="warning-text">Failed to process receipt. Please enter manually.</p>';
...
function displayReceiptResult(data) {
  ...
  html += '<h4>✅ Receipt Scanned Successfully</h4>';
  ...
  if (parsedData.amount || parsedData.date || parsedData.merchant) {
    ...
    html += '</div>';
    html += '<p class="result-question">Use this information?</p>';
    html += '<div class="result-actions">';
    html += '<button class="btn btn-success btn-block" onclick="useReceiptData()">✓ Yes, Use This</button>';
    html += '<button class="btn btn-primary btn-block" onclick="enterManually()">✗ No, Enter Manually</button>';
    html += '</div>';
  } else {
    html += '<p class="warning-text">Could not extract data from receipt. Please enter manually.</p>';
    html += '<button class="btn btn-primary btn-block" onclick="enterManually()">Enter Manually</button>';
  }
